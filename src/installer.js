/**
 * installer.js — Core installation logic for antigravity-skills
 *
...
function installMerged(skillsRoot, skillNames, destFile, { force }) {
  if (!force && fs.existsSync(destFile)) {
    console.log(`  ⚠ ${destFile} already exists (use --force to overwrite)`);
    return;
  }

  let merged = "# AntiGravity Skills\n\n";
  merged +=
    "Auto-generated by `npx antigravity-skills install --tool codex`.\n\n";
...
async function install({
  ...
  for (const toolKey of targets) {
    ...
    if (tool.format === "folder") {
      ...
      if (project && tool.project) {
        const dest = path.resolve(tool.project);
        installMerged(skillsRoot, toInstall, dest, { force });
      } else {
        console.log(
          `  ⚠ ${tool.name} requires --project (project-level install only)`
        );
...
function listSkills() {
  ...
  console.log(
    `\n  Install: npx antigravity-skills install\n`
  );
}
