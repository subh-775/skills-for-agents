"""
Benchmark: ReLU² GLU — torch.compile vs Triton
...
relu2_glu_compiled = torch.compile(relu2_glu_eager)
...
def relu2_glu_triton(gate, up):
    ...
    relu2_glu_kernel[grid](gate, up, out, M, N, gate.stride(0))
    return out
...
if __name__ == "__main__":
    print("=" * 70)
    print("ReLU² GLU Benchmark: torch.compile vs Triton")
    ...
    for _ in range(5): relu2_glu_compiled(g, u)
    torch.cuda.synchronize()
    print("Done.\n")
    ...
    for M, N in shapes:
        ...
        ref = relu2_glu_eager(gate, up)
        tri = relu2_glu_triton(gate, up)
        ok = torch.allclose(ref.float(), tri.float(), atol=1e-2, rtol=1e-2)
        e = bench(relu2_glu_eager, gate, up)
        c = bench(relu2_glu_compiled, gate, up)
        t = bench(relu2_glu_triton, gate, up)
        vs_eager = e / t
        vs_compile = c / t
