...
**PptxGenJS has NO animation or transition support.** Animations are injected
post-generation by `scripts/add_animations.js` using JSZip to modify the OOXML.

### Usage

```bash
# Generate static PPTX first
...
### Animations Config Format

```json
{
  "transitions": {
    "default": { "type": "fade", "speed": "med" },
    "1": { "type": "zoom", "speed": "slow" }
  },
  "animations": {
    "2": [
...
- `transitions` key: slide transitions (between slides), keyed by slide number or `"default"`
- `animations` key: element animations (on slide), keyed by slide number
- `target`: 0-based element index (order of `addText`/`addShape`/`addImage` calls)
- `trigger`: `"withPrev"` (parallel), `"afterPrev"` (sequential), `"onClick"`
- Uses JSZip (already a pptxgenjs dependency) to inject OOXML `<p:transition>` and `<p:timing>` XML
- See `references/animations.md` for full effect catalog
